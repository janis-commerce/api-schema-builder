...
class ApiSchemaBuilder {
	...
	async build() {
		...
		if(!await this._isDirectory(this.constructor.schemaDir))
			return logger('Directory \'schemas/\' don\'t exist. Need to build.', 'ERROR', true);
		...
		if(!await this._isDirectory(this.constructor.schemaSrcDir))
			return logger('Directory \'schemas/src/\' don\'t exist. Need to build.', 'ERROR', true);
		...
		try {
			...
			if(!schemaTypes.length)
				return logger('No Files to Build', 'ERROR', true);
			...
			return logger(error.message, 'ERROR', true);
