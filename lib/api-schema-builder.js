...
// const ApiSchemaBuilderError = require('./api-schema-builder-error');
...
const logger = (message, prefix = 'BUILDING', error = false) => {
	...
	if(error) {
		...
		console.error(`[\x1b[35m \x1b[1mAPI-SCHEMA-BUILDER\x1b[0m | ${time} ] | \x1b[31mERROR\x1b[0m | Abort. Can't create ENVIRONMENT.\n`);
	} else
...
class ApiSchemaBuilder {
	...
	/**
	 * Retrieves the schemas source tree path
	 * @async
	 * @param {string} directory
		...
		for(const element of content) {
			...
			if(isDir) {
				...
				if(!elementTree) {
					...
					tree.schemas = tree.schemas || [];
					set(tree, 'schemas', tree.schemas.concat(elementPath));
				} else {
					// mutate the tree object
					set(tree, `${parentPropertyChain}.schemas`, elementTree.schemas.concat(elementPath));
				} */
			}
		}
	 ...
	 */
	_parseFile(fileType, fileContent) {
		switch(fileType) {
			case 'yml':
			case 'yaml':
				try {
	 ...
	 */
	async _readSchemaFiles(schemaPaths) {
		const files = await Promise.all(schemaPaths.map(async pathname => {
			const fileType = pathname.split('.').pop();
			const file = await fs.readFile(pathname, 'utf8');

			try {
				const parsedFile = this._parseFile(fileType, file);
	...
	async _validateSchema(schemaType, schema) {
		...
		try {
			await fs.writeFile(path.join(this.constructor.schemaDir, 'build-error.log'), JSON.stringify({ [schemaType]: results }, null, '\t'));
	...
	async _buildSchema(schemaType, schemaTree) {
		...
		const resolvedSchemaResult = await jsonRefs.resolveRefs(schema, {
			...
			location: `${this.constructor.schemaDir}${schemaType}.json`
		});
		...
		try {
			...
			throw new Error('Can\'t make the file public.json.');
		}
	}
